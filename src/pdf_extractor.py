...
def _identify_table_type(header_rows: list, ncols: int) -> Optional[str]:
    ...
    Returns:
        'no_withdrawal_cn' — Chinese 12-col combined surrender + death table
        'withdrawal_cn'    — Chinese 9-col withdrawal-after-surrender table
        'no_withdrawal'    — legacy surrender value table (8 cols)
        'death_benefit'    — legacy death benefit table (10 cols)
        'withdrawal'       — legacy withdrawal scenario table (10 cols)
        None               — unknown
    """
    flat = ' '.join(
        decode_cid(str(cell)) for row in header_rows for cell in row if cell
    )
    flat_norm = re.sub(r'\s+', '', flat)

    # Newer Chinese table formats
    if (
        ncols >= 11
        and '现金提取后' not in flat_norm
        and all(
            kw in flat_norm
            for kw in ['年龄', '保单年度终结', '缴付保费总额', '退保发还金额', '身故赔偿额']
        )
    ):
    ...
    if (
        ...
        and all(
            kw in flat_norm
            for kw in ['年龄', '保单年度终结', '现金提取金额', '现金提取后之退保发还金额']
        )
    ):
...
class AIAPDFExtractor:
    ...
    def _do_extract(self) -> dict:
        ...
        nw_tables = []       # [(table_type, table)]
        db_tables = []       # [(table_type, table)]
        wd_tables = []       # [(table_type, table)]
        ...
        for page_idx, page in enumerate(self._pdf.pages):
            ...
            for table in tables:
                ...
                if ttype in ('no_withdrawal', 'no_withdrawal_cn'):
                    nw_tables.append((ttype, table))
                elif ttype == 'death_benefit':
                    db_tables.append((ttype, table))
                elif ttype in ('withdrawal', 'withdrawal_cn'):
                    wd_tables.append((ttype, table))

        # 2. Extract policy info from page 1
        policy_info = self._extract_policy_info(policy_tables)

        # 3. Extract yearly data
    ...
    @staticmethod
    def _iter_typed_tables(tables: list, default_type: str):
        """Yield (table_type, table) while preserving old call compatibility."""
        for item in tables:
            if isinstance(item, tuple) and len(item) >= 2 and isinstance(item[0], str):
                yield item[0], item[1]
            else:
                yield default_type, item
    ...
    def _extract_policy_info(self, policy_tables: list) -> dict:
        ...
        # 1) Identity fields from first small table + text labels
        if policy_tables and policy_tables[0]:
            first_row = [decode_cid(str(c)).strip() if c else '' for c in policy_tables[0][0]]
            first_row_text = ' '.join(c for c in first_row if c)
            ...
            if name_match:
                info['insured_name'] = name_match.group(1).strip(' -')

            age_match = re.search(r'年龄[:：]\s*(\d{1,3})', first_row_text)
            if age_match:
                age = int(age_match.group(1))
                if 0 <= age <= 120:
                    info['age_at_issue'] = age

            gender_match = re.search(r'性别[:：]\s*([男女MF])', first_row_text)
            if gender_match:
                g = gender_match.group(1)
                info['gender'] = 'M' if g in ('男', 'M') else 'F'

        if not info['insured_name']:
            name_match = re.search(
                r'受保人姓名[:：]\s*([^\n]+?)(?:\s+年龄[:：]|\s+性别[:：]|$)',
                page1_decoded,
            )
        ...
        if info['age_at_issue'] == 0:
            age_match = re.search(r'年龄[:：]\s*(\d{1,3})', page1_decoded)
            if age_match:
                age = int(age_match.group(1))
                if 0 <= age <= 120:
                    info['age_at_issue'] = age
        ...
        if info['gender'] not in ('M', 'F'):
            gender_match = re.search(r'性别[:：]\s*([男女MF])', page1_decoded)
            if gender_match:
                g = gender_match.group(1)
                info['gender'] = 'M' if g in ('男', 'M') else 'F'

        # 2) Annual premium + payment years from policy summary table by header labels
        for table in policy_tables:
            if len(table) < 2:
                continue

            header = [decode_cid(str(c)).strip() if c else '' for c in table[0]]
            values = [decode_cid(str(c)).strip() if c else '' for c in table[1]]
            ...
            for idx, h in enumerate(header_norm):
                ...
                if '年缴保费' in h and info['annual_premium'] == 0:
                    parts = [p.strip() for p in cell.split('\n') if p.strip()]
                    nums = [clean_numeric(p) for p in parts]
                    nums = [n for n in nums if n > 0]
                    if nums:
                        info['annual_premium'] = max(nums)

                if '保费供款年期' in h:
                    for token in re.findall(r'\d{1,2}', cell):
                        py = int(token)
                        if 2 <= py <= 30:
    ...
    def _extract_yearly_data(self, nw_tables: list, db_tables: list,
        ...
        # Build death benefit lookup from legacy death table (if present)
        db_rows = []
        for ttype, table in self._iter_typed_tables(db_tables, 'death_benefit'):
            if ttype != 'death_benefit':
                continue
        ...
        for ttype, table in self._iter_typed_tables(nw_tables, 'no_withdrawal'):
            nw_rows = _expand_rows(table, header_rows=3)

            for row in nw_rows:
                if ttype == 'no_withdrawal_cn':
                    # Chinese 12-col table:
                    # Age, Year, Cumul.Premium, (A), (B), (C), Total, ..., ..., ..., DeathTotal
                    if len(row) < 7:
                        continue
                    ...
                    age = int(row[0]) if row[0].strip().isdigit() else age_at_issue + year
                    cumulative_premium = clean_numeric(row[2]) if len(row) > 2 else 0
                    guaranteed_cv = clean_numeric(row[3]) if len(row) > 3 else 0
                    reversionary_bonus = clean_numeric(row[4]) if len(row) > 4 else 0
                    terminal_dividend = clean_numeric(row[5]) if len(row) > 5 else 0
                    total_surrender = clean_numeric(row[6]) if len(row) > 6 else 0

                    if cumulative_premium == 0 and annual_premium > 0:
                        cumulative_premium = annual_premium * min(year, payment_years)

                    death_benefit = clean_numeric(row[11]) if len(row) > 11 else 0
                    if death_benefit == 0 and len(row) > 10:
                        death_benefit = clean_numeric(row[10])
                    if death_benefit == 0:
                        death_benefit = max(total_surrender, cumulative_premium)

                else:
                    # Legacy 8-col table:
                    # Year, Age, Cumul.Premium, (A), (B), (C), #(D), (E)Total
                    if len(row) < 8:
                        continue
                    year_str = row[0].strip()
                    if not year_str.isdigit():
    ...
    def _extract_withdrawal_data(self, wd_tables: list) -> list[dict]:
        ...
        for ttype, table in self._iter_typed_tables(wd_tables, 'withdrawal'):
            wd_rows = _expand_rows(table, header_rows=3)

            for row in wd_rows:
                if ttype == 'withdrawal_cn':
                    # Chinese 9-col table:
                    # Age, Year, Cumul.Premium, Withdrawal, BasicAfterWD, (A), (B), (C), Total
                    if len(row) < 9:
                        continue
                    year_str = row[1].strip()
                    if not year_str.isdigit():
                    ...
                    remaining_total = clean_numeric(row[8])

                else:
                    # Legacy 10-col table:
                    # Year, Age, ..., Withdrawal, ..., (A), (B), (C), #(D), Total
                    if len(row) < 10:
                        continue
                    year_str = row[0].strip()
                    if not year_str.isdigit():
        ...
        withdrawal_data.sort(key=lambda x: x['year'])
